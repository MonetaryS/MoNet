...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MoNet
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MoNet
    // Mac: ~/Library/Application Support/MoNet
    // Unix: ~/.monet
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MoNet";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "monet.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "monetd.pid"));
