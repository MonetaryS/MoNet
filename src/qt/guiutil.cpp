...
{
    // Convert monet:// to monet:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("monet://"))
    {
        uri.replace(0, 12, "monet:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "MoNet.lnk";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart)
    {
        ...
        // Get a pointer to the IMoNetLink interface.
        IShellLink* psl = NULL;
        ...
        if (SUCCEEDED(hres))
        {
            ...
            // Query IMoNetLink for the IPersistFile interface for
...
{
    return GetAutostartDir() / "monet.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("MoNet-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  monet-qt [" + tr("command-line options") + "]                     " + "\n";
