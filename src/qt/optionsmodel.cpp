...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeMoNetAmount"))
        settings.setValue("nAnonymizeMoNetAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeMoNetAmount = settings.value("nAnonymizeMoNetAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeMoNetAmount"))
        SoftSetArg("-anonymizemonetamount", settings.value("nAnonymizeMoNetAmount").toString().toStdString());
}
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizeMoNetAmount:
            nAnonymizeMoNetAmount = value.toInt();
            settings.setValue("nAnonymizeMoNetAmount", nAnonymizeMoNetAmount);
            emit anonymizeMoNetAmountChanged(nAnonymizeMoNetAmount);
            break;
        default:
            break;
        }
