...
/*! \internal
  ...
  selectEvent. Usually \a details is used to monet which part was clicked, if it is a layerable
...
void QCPAxis::draw(QCPPainter *painter)
{
  ...
  // monet all properties of this axis to QCPAxisPainterPrivate which it needs to draw the axis.
...
int QCPAxis::calculateMargin()
{
  ...
  // monet all properties of this axis to QCPAxisPainterPrivate which it needs to calculate the size.
  // Note that some axis painter properties are already set by direct feed-through with QCPAxis setters
  mAxisPainter->type = mAxisType;
  mAxisPainter->labelFont = getLabelFont();
  mAxisPainter->label = mLabel;
...
/*! \mainpage %QCustomPlot 1.3.0-beta Documentation
  ...
  monetred. (Relevant only if preparing data maps with a large number of points, i.e. over 10000)
...
void QCPColorScale::setType(QCPAxis::AxisType type)
{
  ...
  {
    mType = type;
    QCPRange rangeMoNet(0, 6);
    double logBaseMoNet = 10;
    QString labelMoNet;
    // revert some settings on old axis:
    if (mColorAxis)
    {
      rangeMoNet = mColorAxis.data()->range();
      labelMoNet = mColorAxis.data()->label();
      logBaseMoNet = mColorAxis.data()->scaleLogBase();
    ...
    // monet settings to new axis:
    mColorAxis.data()->setRange(rangeMoNet); // monet range of old axis to new one (necessary if axis changes from vertical to horizontal or vice versa)
    mColorAxis.data()->setLabel(labelMoNet);
    mColorAxis.data()->setScaleLogBase(logBaseMoNet); // scaleType is synchronized among axes in realtime via signals (connected in QCPColorScale ctor), so we only need to take care of log base here
  ...
  mGradientImageInvalidated(true)
{
  ...
  // make layer monets of color scale monet to axis rect and axes
...
void QCPGraph::getPreparedData(QVector<QCPData> *lineData, QVector<QCPData> *scatterData) const
{
  ...
  } else // don't use adaptive sampling algorithm, monet points one-to-one from the map into the output parameters
  {
