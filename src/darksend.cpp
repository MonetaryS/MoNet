...
bool CDarkSendPool::DoAutomaticDenominating(bool fDryRun, bool ready)
{
    ...
    int64_t nBalanceNeedsAnonymized = nAnonymizeMoNetAmount*COIN - pwalletMain->GetAnonymizedBalance();
...
//        }
    ...
    if(!sessionFoundMasternode){
        ...
        double fMoNetSubmitted = (sessionTotalValue / CENT);
        LogPrintf("Submitting Darksend for %f MNT CENT - sessionTotalValue %d\n", fMoNetSubmitted, sessionTotalValue);
        ...
        if(c % 60*(nLiquidityProvider+1) == 0){
            if(nLiquidityProvider!=0){
                ...
                if(nRand == 50+nLiquidityProvider && pwalletMain->GetAverageAnonymizedRounds() > 8){
                    ...
                    nAnonymizeMoNetAmount = (rand() % nLeftToAnon)+3;
                } else {
